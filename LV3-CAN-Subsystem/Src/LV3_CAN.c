...
LV3_CAN_Result LV3_CAN_Init(uint8_t _this_module_id,
                            LV3_CAN_BusMode _bus_mode,
...
LV3_CAN_Result LV3_CAN_Loop() {
	...
	for (unsigned int i = 0; i < num_bindings; i++) {
		...
		if (binding->bind_mode == LV3_CAN_BindMode_Write) {
			if ((LV3_CAN_Driver_GetTick() - state->last_sent_time) * 2 > metadata->ttl ||
			    (state->last_sent_value != *binding->user_var &&
			     state->last_sent_time + metadata->refresh_interval < LV3_CAN_Driver_GetTick())) {
				state->flag_for_send = 1;
				// for now, just send no matter what
				LV3_CAN_Result res =
				    LV3_CAN_Driver_SendMessage(metadata->can_id.raw, (uint8_t*)binding->user_var, sizeof(uint32_t));
				if (res == LV3_CAN_OK) {
					state->last_sent_value = *binding->user_var;
		...
		if (binding->bind_mode == LV3_CAN_BindMode_Read) {
			if (state->last_received_time + metadata->ttl < LV3_CAN_Driver_GetTick()) {
				stale_data = 1;
				state->is_stale = 1;
	...
	if (!heartbeat_reset_occurred &&
	    (LV3_CAN_Driver_GetTick() - heartbeat_timestamp > (LV3_CAN_HEARTBEAT_PULSE + this_module_id))) {
		heartbeat_counter = 0;
		heartbeat_pulse_in_progress = 0;
		heartbeat_reset_occurred = 1;
	}
	if (LV3_CAN_Driver_GetTick() - heartbeat_timestamp > (LV3_CAN_HEARTBEAT_INTERVAL + this_module_id)) {
		// send heartbeat
		LV3_CAN_CompositeID heartbeat_id = {.elements = {.sender = this_module_id,
		                                                 .parameter = 0,
		                                                 .message_type = LV3_CAN_MessageType_Heartbeat,
		                                                 .priority = LV3_CAN_LOWEST_PRIORITY,
		                                                 ._reserved = 0}};
		LV3_CAN_Result res = LV3_CAN_Driver_SendMessage(heartbeat_id.raw, 0, 0);
		if (res != LV3_CAN_OK) {
			tx_error = 1;
		} else {
			heartbeat_timestamp = LV3_CAN_Driver_GetTick();
