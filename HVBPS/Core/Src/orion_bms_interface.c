...
// It runs at a different baud rate than the main LV3 vehicle CAN (FDCAN1)
// and carries proprietary Orion BMS2 broadcast messages, not LV3 protocol
// frames.
...
// so IR and open-voltage are available for future diagnostics without
// re-parsing. Packed to match the Orion BMS2 wire layout for direct cast in the
// RX callback.
...
int32_t reported_pack_current =
    0; // conversion of pack_current to int32_t for LV3 CAN binding
...
int32_t min_thermistor_temp = 0;

// Aux CAN message rate. Every frame the BMS sends is counted in the RX
// callback; OrionBMS_Loop reads and clears the counter once per window and
// scales it up to messages/second.
#define ORION_RATE_WINDOW_MS 250
static volatile uint32_t orion_rx_count = 0;
static uint32_t orion_rate_last_tick = 0;

uint32_t orion_message_rate = 0;
...
void LV3_CAN_AUX_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan,
  ...
  // Count every frame off the bus, including IDs we don't decode below — this
  // is a link-health metric, not a decode-success metric.
  orion_rx_count++;
  ...
  // 0x101: current limits and live current/DOD. Reconstruct the 16-bit
  // values explicitly so the byte ordering matches the other frames.
  case 0x101: {
    pack_ccl = (rx_data[0] << 8) | rx_data[1];
    pack_dcl = (rx_data[2] << 8) | rx_data[3];
    pack_current = (int16_t)((rx_data[4] << 8) | rx_data[5]);
    pack_dod = (rx_data[6] << 8) | rx_data[7];

    reported_pack_current =
        (int32_t)pack_current; // sign-extend for LV3 CAN binding
  ...
  case 0x36: {
    ...
    if (idx < 24) {
      ...
      cell_voltage_data[idx].internal_resistance =
  ...
  case 0x76: {
    uint8_t idx = rx_data[0];
    if (idx < 8)
      thermistor_temp[idx] = (int8_t)rx_data[1];
    break;
  }
  }
}
...
void OrionBMS_Init(void) {
  ...
  orion_rate_last_tick = HAL_GetTick();
}
...
void OrionBMS_Loop(void) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (thermistor_temp[i] > max)
      max = thermistor_temp[i];
    if (thermistor_temp[i] < min)
      min = thermistor_temp[i];
  }
  average_thermistor_temp = sum / 8;
  max_thermistor_temp = max;
  min_thermistor_temp = min;

  // Message rate: messages counted in the last window, scaled to per-second.
  // The main loop never blocks, so the window closes promptly at 250 ms and
  // the x4 scaling holds.
  uint32_t now = HAL_GetTick();
  if (now - orion_rate_last_tick >= ORION_RATE_WINDOW_MS) {
    orion_message_rate = orion_rx_count * (1000 / ORION_RATE_WINDOW_MS);
    orion_rx_count = 0;
    orion_rate_last_tick = now;
  }
}

// OBD2 service 0x04 (clear emission-related DTCs) is the command the Orion BMS2
// uses to reset its fault flags. ID 0x7E3 is the Orion BMS2 diagnostic request
// ID.
