...
const LV3_CAN_Binding lv3_can_bindings[] = {
    {&main_switch, SW_HV_MAIN, LV3_CAN_BindMode_Write}};

#define NUM_BINDINGS (sizeof(lv3_can_bindings) / sizeof(lv3_can_bindings[0]))
...
int main(void) {
  ...
  LV3_CAN_Init(99, LV3_CAN_BusMode_Normal, lv3_can_bindings,
               sizeof(lv3_can_bindings) / sizeof(LV3_CAN_Binding));

  // LV3_CAN_ForceSend(sw)
  ...
  while (1) {
    ...
    /* USER CODE BEGIN 3 */
    // read btn usr
    main_switch = HAL_GPIO_ReadPin(BTN_USR_GPIO_Port, BTN_USR_Pin);
    LV3_CAN_Loop();
  }
  /* USER CODE END 3 */
}
...
static void MX_GPIO_Init(void) {
  ...
  /*Configure GPIO pin : AUX_CAN_RX_Pin */
  ...
  /*Configure GPIO pin : AUX_CAN_TX_Pin */
