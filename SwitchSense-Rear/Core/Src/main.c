...
// --- RPM (IN_10 pulse input) ---
// Adaptive: at high speed enough pulses land in each update window to count
// them (averages out HAL_GetTick()'s 1ms quantization); at low speed too few
// pulses land per window, so fall back to timing the single last inter-pulse
// period instead.
#define RPM_PULSES_PER_REV      48u   // pulses per wheel revolution
#define RPM_UPDATE_INTERVAL_MS  100u  // recompute rpm at this rate
#define RPM_TIMEOUT_MS          500u  // no pulse for this long => report 0 RPM
#define RPM_MIN_COUNT_FOR_FREQ  2u    // below this many pulses/window, use period instead

volatile uint32_t rpm_pulse_count = 0;     // pulses since last RPM update
volatile uint32_t rpm_last_pulse_tick = 0; // tick of most recent pulse
volatile uint32_t rpm_last_period_ms = 0;  // ms between the last two pulses

uint32_t rpm_update_tick = 0;
float rpm = 0.0f;

// --- Speed (derived from RPM and wheel size) ---
#define WHEEL_DIAMETER_IN  21.5f  // configurable wheel diameter, inches
// mph = rpm * (pi * diameter) [inches/rev] * 60 [min/hr] / 63360 [inches/mile]
#define RPM_TO_MPH_SCALE   60.0f * (WHEEL_DIAMETER_IN * 3.14159265f / 12.0f / 5280.0f)

float speed_mph = 0.0f;

// Basic first-order IIR (exponential moving average) low-pass on the speed
// output, to smooth out the update-to-update noise from the RPM calc.
#define SPEED_MPH_IIR_ALPHA  0.2f  // 0-1; smaller = smoother but slower to respond
float speed_mph_filtered = 0.0f;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    adc_pin_voltage = (V_SENSE_12 / (4095.0f * 64.0f / 4.0f)) * 3.3f;
    ...
    // RPM: below minimum speed (no pulse within timeout) reports zero. Above
    // that, use pulse counting over the update window at high speed (where
    // enough pulses land per window for good precision) and fall back to the
    // last measured inter-pulse period at low speed (too few pulses/window).
    uint32_t rpm_now = HAL_GetTick();
    if (rpm_now - rpm_last_pulse_tick >= RPM_TIMEOUT_MS) {
      rpm = 0.0f;
      rpm_pulse_count = 0;
      rpm_update_tick = rpm_now;
    } else if (rpm_now - rpm_update_tick >= RPM_UPDATE_INTERVAL_MS) {
      __disable_irq();
      uint32_t count = rpm_pulse_count;
      rpm_pulse_count = 0;
      uint32_t period_ms = rpm_last_period_ms;
      __enable_irq();

      uint32_t window_ms = rpm_now - rpm_update_tick;
      rpm_update_tick = rpm_now;

      if (count >= RPM_MIN_COUNT_FOR_FREQ) {
        rpm = ((float)count / RPM_PULSES_PER_REV) * (60000.0f / (float)window_ms);
      } else if (period_ms > 0) {
        rpm = 60000.0f / ((float)period_ms * RPM_PULSES_PER_REV);
      }
    }

    speed_mph = rpm * RPM_TO_MPH_SCALE;
    speed_mph_filtered += SPEED_MPH_IIR_ALPHA * (speed_mph - speed_mph_filtered);

    // --- LV3 CAN ---
    motor_rpm_val = (uint32_t)lroundf(rpm);
    motor_speed_val = (uint32_t)lroundf(speed_mph_filtered);
    LV3_CAN_Loop();

    // Toggle OUT_1 every 1s
    if (HAL_GetTick() - out_1_toggle_tick >= 1000) {
      out_1_toggle_tick = HAL_GetTick();
      out_1_state ^= 1;
    }
    ...
    if (_hv_faults) {
      HAL_GPIO_WritePin(OUT_2_GPIO_Port, OUT_2_Pin, (HAL_GetTick() / 200) % 2);
      HAL_GPIO_WritePin(OUT_3_GPIO_Port, OUT_3_Pin, (HAL_GetTick() / 200) % 2);
    } else {
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, OUT_3_Pin|OUT_1_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : OUT_3_Pin OUT_1_Pin */
  GPIO_InitStruct.Pin = OUT_3_Pin|OUT_1_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  /*Configure GPIO pins : IN_7_Pin IN_8_Pin IN_9_Pin */
  GPIO_InitStruct.Pin = IN_7_Pin|IN_8_Pin|IN_9_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  /*Configure GPIO pin : IN_10_Pin */
  GPIO_InitStruct.Pin = IN_10_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(IN_10_GPIO_Port, &GPIO_InitStruct);
...
{
  if (GPIO_Pin == IN_10_Pin) {
    uint32_t now = HAL_GetTick();
    if (rpm_last_pulse_tick != 0) {
      rpm_last_period_ms = now - rpm_last_pulse_tick;
    }
    rpm_last_pulse_tick = now;
    rpm_pulse_count++;
  }
}
